...
def _no_auto_ingest(monkeypatch):
    """所有 CLI 测试阻断自动入库（避免加载真实 embedding 模型 bge-m3）。"""
...
def test_full_read_cache_roundtrip(monkeypatch, tmp_cache, sample_pdf):
    ...
    monkeypatch.setattr(cli, "build_paper_graph", lambda: FakeGraph())
...
def test_ask_with_mock_rag(monkeypatch, tmp_cache):
    ...
    monkeypatch.setattr(cli, "build_rag_graph", lambda: FakeRagGraph())
